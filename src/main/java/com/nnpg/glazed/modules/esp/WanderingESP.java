...
public class WanderingESP extends Module {
    ...
    private void handleTraderDetection(int traderCount) {
        ...
        switch (notificationMode.get()) {
            ...
            case Toast -> mc.getToastManager().addToast(new MeteorToast(Items.EMERALD, title, message));
            case Both -> {
                if (notifications.get()) info("(highlight)%s", message);
                mc.getToastManager().addToast(new MeteorToast(Items.EMERALD, title, message));
            }
        }
