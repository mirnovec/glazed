            ...
            if (showCount.get() && pillagers.size() != lastPillagerCount) {
                if (pillagers.size() > 0) {
                    ...
                    switch (notificationMode.get()) {
                        ...
                        case Toast -> mc.getToastManager().addToast(new MeteorToast(Items.CROSSBOW, title, message));
                        case Both -> {
                            if (notifications.get()) info("§5[§dPillagerESP§5] §c" + message);
                            mc.getToastManager().addToast(new MeteorToast(Items.CROSSBOW, title, message));
                        }
                    }
                }
