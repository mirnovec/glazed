...
public class EmergencyOrder extends Module {
    ...
    private void fire(Item wanted, String spotted) {
        ...
        if (notify.get()) info("%s is close. Ordering %s to %s.", triggeredBy, wanted.getName().getString(), target);
    ...
    private void pickItem(AbstractContainerMenu handler, String title, Item wanted) {
        ...
        click(handler, slot, ClickType.PICKUP);
        step = Step.OPENING;
        waitTicks = guiDelay.get();
    }
    ...
    private void fillDelivery(AbstractContainerMenu handler, String title, Item wanted) {
        ...
        if (!cursor.isEmpty() && cursor.is(wanted)) {
            ...
            if (empty != -1) {
                click(handler, empty, ClickType.PICKUP);
                waitTicks = guiDelay.get();
            }
            return;
        }
        ...
        click(handler, mine, ClickType.QUICK_MOVE);
        waitTicks = guiDelay.get();
    }
    ...
    private void confirm(AbstractContainerMenu handler, String title) {
        ...
        click(handler, button, ClickType.PICKUP);
        clear();
    }
    ...
    private void click(AbstractContainerMenu handler, int slotId, ClickType action) {
        if (mc.gameMode == null || mc.player == null) return;
        mc.gameMode.handleInventoryMouseClick(handler.containerId, slotId, 0, action, mc.player);
    }
