...
public final class GlazedSell {
    ...
    public static boolean clickConfirm(ChestMenu handler) {
        ...
        mc.gameMode.handleInventoryMouseClick(handler.containerId, slot, 0, ClickType.PICKUP, mc.player);
        return true;
    }
    ...
    public static boolean depositNext(ChestMenu handler) {
        ...
        for (int slot = containerSlots; slot < handler.slots.size(); slot++) {
            ...
            mc.gameMode.handleInventoryMouseClick(handler.containerId, slot, 0, ClickType.QUICK_MOVE, mc.player);
            return true;
        }
    ...
    // server dialogs are 1.21.6+ only, so both of these are hard false here. callers already
    // fall through to the chest confirm
